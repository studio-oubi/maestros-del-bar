...
describe("validarRegistro", () => {
  it("acepta y normaliza un registro válido", () => {
    ...
    expect(r).toEqual({
      ok: true,
      datos: { nombre: "Ana Pérez", cedula: "001-1234567-8", telefono: "(809) 555-1234", correo: "ana@mail.com", ciudad: "", establecimiento: "" },
    });
  });
  ...
  it("correo vacío es válido", () => {
    ...
    expect(r).toEqual({
      ok: true,
      datos: { nombre: "Ana", cedula: "001-1234567-8", telefono: "(809) 555-1234", correo: "", ciudad: "", establecimiento: "" },
    });
  });
  ...
  it("ciudad y establecimiento ausentes normalizan a cadena vacía", () => {
    const r = validarRegistro({ nombre: "Ana", cedula: "00112345678", telefono: "8095551234" });
    expect(r).toEqual({
      ok: true,
      datos: {
  ...
  it("ciudad y establecimiento se conservan cuando vienen con valor", () => {
    const r = validarRegistro({
      nombre: "Ana",
      cedula: "00112345678",
      telefono: "8095551234",
      ciudad: "Santo Domingo",
      establecimiento: "Marketcito",
    });
    expect(r.ok).toBe(true);
    if (r.ok) {
      expect(r.datos.ciudad).toBe("Santo Domingo");
      expect(r.datos.establecimiento).toBe("Marketcito");
    }
  });
